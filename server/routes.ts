...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/admin/assign-rooms", async (req, res) => {
    try {
      ...
      // Use callback-based fs.readFile and wrap in Promise
      const biddingAssignmentsContent = await new Promise<string>((resolve, reject) => {
        fs.readFile(BIDDING_ASSIGNMENTS_CSV_PATH, 'utf8', (err, data) => {
          if (err) reject(err);
          else resolve(data);
        });
      });
